...
- The CTAN package, submission directory, and release archive are named
  `geist-fonts`.
...
- Documentation must distinguish the `geist-fonts` CTAN distribution from the
  `geist` and `geistmono` LaTeX packages.

## CTAN archive

- Build and upload only the flat CTAN tree, with one top-level
  `geist-fonts/` directory and the file-type directories `doc/`, `latex/`,
...
- A GitHub release must publish `geist-fonts.zip` as its CTAN archive asset;
