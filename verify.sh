...
#
# Installs dist/geist.tds.zip into a throwaway texmf tree and compiles the
# documentation with every engine the package claims to support.  The tree is
# thrown away so nothing here can be satisfied by accident from the working
# directory or from a real installation.

set -euo pipefail
cd "$(dirname "$0")"
ROOT=$PWD

[ -f dist/geist.tds.zip ] || { echo "error: run build.sh first" >&2; exit 1; }
...
mkdir -p "$TEXMFHOME"
unzip -q "$ROOT/dist/geist.tds.zip" -d "$TEXMFHOME"
